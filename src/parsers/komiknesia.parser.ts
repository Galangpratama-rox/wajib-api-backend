import type * as T from "@interfaces/komiknesia.interface.js";

const komiknesiaParser = {
  /**
   * Normalize raw IKomiknesiaContent → IKomikCard
   */
  parseKomikCard(raw: T.IKomiknesiaContent): T.IKomikCard {
    return {
      id: raw.id,
      title: raw.title,
  ...
  parseKomikList(raw: T.IKomiknesiaListResponse): {
    komikList: T.IKomikCard[];
   ...
   */
  parseKomikDetails(raw: T.IKomiknesiaComicDetailRaw): T.IKomikDetails {
    return {
      id: raw.id,
      title: raw.title,
      alternativeTitle: raw.alternative_name || "",
   ...
   */
  parseChapterDetails(
    raw: T.IKomiknesiaChapterReadRaw,
    komikSlug: string
  ): T.IChapterDetails {
    return {
      number: raw.number || "",
