...
export interface DataServiceOptions {
  /** Tipe provider — menentukan limit dan timeout yang dipakai */
  type: "scrape" | "api";
  /** TTL Redis dalam detik. Default: TTL_SCRAPE atau TTL_API sesuai type */
  ttl?: number;
  /**
   * Kalau true, simpan stale copy dan kembalikan data lama kalau provider gagal.
   * Aktifkan untuk data metadata (home, list, detail).
   * Nonaktifkan untuk data ephemeral (episode stream URL, chapter images).
   */
  allowStale?: boolean;
}
...
export async function fetchWithCache<T>(
  ...
  const promise = limiter(async (): Promise<T> => {
  ...
  try {
    ...
    // ── 5. Stale fallback ──────────────────────────────────────────────────
    if (allowStale) {
      const staleData = await getStaleCached<T>(cacheKey);
      if (staleData !== null) {
        console.warn(`[dataService] provider failed for "${cacheKey}", returning stale data:`, (err as Error)?.message);
