...
export function isBlockedContent(opts: {
  ...
  slug?: string;
  url?: string;
  genreList?: { title?: string; propertyId?: string }[];
  explicit?: string;
}): boolean {
 ...
 */
export function filterAnimeList<T extends {
  title: string;
 ...
 */
export function shouldBlock(title: string, slug?: string | undefined, url?: string | undefined): boolean {
  return isBlockedContent({
    title,
